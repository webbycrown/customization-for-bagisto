    ...
    >
        ...
        @if( $categorys && count( $categorys ) > 0 )
            ...
            @foreach ( $categorys as $category )
                ...
                @php $selected_val = ( is_array( $section_field_val ) && in_array( $category->id, $section_field_val ) ) ? 'selected' : ''; @endphp

                <option value="{{ $category->id }}" {{ $selected_val }}>{{ $category->name }}</option>
